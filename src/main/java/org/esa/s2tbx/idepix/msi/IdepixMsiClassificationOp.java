...
public class IdepixMsiClassificationOp extends Operator {
...
//                    }
                    ...
                    IdepixMsiAlgorithm s2MsiAlgorithm = createS2MsiAlgorithm(s2ReflectanceTiles,
    ...
    private IdepixMsiAlgorithm createS2MsiAlgorithm(Tile[] s2MsiReflectanceTiles,
                                                   ...
                                                   int y,
                                                   int x) {
        IdepixMsiAlgorithm s2MsiAlgorithm = new IdepixMsiAlgorithm();
...
//        setIsWaterByFraction(watermaskFraction, s2MsiAlgorithm);
    ...
    private boolean isLandPixel(int x, int y, int waterFraction, IdepixMsiAlgorithm s2MsiAlgorithm) {
        if (getGeoPos(x, y).lat > WATER_MASK_SOUTH_BOUND) {
            ...
            if (waterFraction <= 100) {
                ...
                return s2MsiAlgorithm.aPrioriLandValue() > IdepixMsiAlgorithm.LAND_THRESH;
            }
        } else {
            return s2MsiAlgorithm.aPrioriLandValue() > IdepixMsiAlgorithm.LAND_THRESH;
        }
...
//            IdepixUtils.setNewBandProperties(glintRiskBand, "GLINT_RISK", "dl", IdepixConstants.NO_DATA_VALUE, true);
    ...
    void setPixelSamples(Band band, Tile targetTile, int y, int x, IdepixMsiAlgorithm s2Algorithm) {
        // for given instrument, compute more pixel properties and write to distinct band
        if (band == brightBand) {
    ...
    void setCloudFlag(Tile targetTile, int y, int x, IdepixMsiAlgorithm s2Algorithm) {
    ...
    public static class Spi extends OperatorSpi {
        ...
        public Spi() {
            super(IdepixMsiClassificationOp.class);
        }
    }
}
